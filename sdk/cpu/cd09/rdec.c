...
static void rdec_reg_log_info(int rdec_cfg_id)
{
    ...
    log_info("rdec%d->con = 0x%x\n", rdec_cfg_id, rdec_reg->CON);
    log_info("rdec%d->dat = 0x%x\n", rdec_cfg_id, rdec_reg->DAT);
    log_info("rdec%d->smp = 0x%x\n", rdec_cfg_id, rdec_reg->SMP);
    log_info("rdec%d->dbe = 0x%x\n", rdec_cfg_id, rdec_reg->DBE);
    log_info("rdec%d_clk = %d\n", rdec_cfg_id, RDEC_CLK);
}
...
static void rdec_timer_test_func()
{
    ...
    data += dat;
    log_debug("data = %d, dat = %d\n", data, dat);
}
void rdec_test_func()
{
    log_debug("func:%s(), line:%d\n", __func__, __LINE__);
    sys_timer_add(NULL, rdec_timer_test_func, 1000); //1s读取一次编码器值
