...
static int _norflash_init(const char *name, struct norflash_dev_platform_data *pdata)
{
    ...
    if (_norflash.spi_num == (int) - 1) {
        ...
        SPI_OSMutexCreate(&_norflash.mutex);
        _norflash.max_end_addr = 0;
        _norflash.part_num = 0;
        _norflash.spi_pdata_use = pdata->spi_pdata;
    }
...
LSB_CRITICAL_HANDLE_REG(spi_norflash, clock_critical_enter, clock_critical_exit);
...
{
    int reg = 0;
    SPI_OSMutexPend(&_norflash.mutex, 0);
    log_info("norflash open\n");
    if (!_norflash.open_cnt) {
        spi_cs_init();
        spi_open(_norflash.spi_num, _norflash.spi_pdata_use);
...
__exit:
    SPI_OSMutexPost(&_norflash.mutex);
    return reg;
}
...
static int _norflash_write(u32 addr, void *buf, u32 len, u8 cache)
{
    ...
    if ((addr + align_len) % 4096) {
        ...
        if (flash_cache_timer) {
            sys_timer_reset(flash_cache_timer);
        } else {
...
{
    return _norflash_init(name, pdata);
}
