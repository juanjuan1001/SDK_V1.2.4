...
static void (*hw_spi_irq_cbfun[HW_SPI_MAX_NUM])(hw_spi_dev spi, enum hw_spi_isr_status sta);
#if 0
...
#ifdef SPI_HW_FIXED_IO
const u8 spi1_fixed_io_tab[SPI1_HW_FIXED_IO_GROUP_NUM][5] = {
    {SPI1_HW_FIXED_IO_A},
    {SPI1_HW_FIXED_IO_B},
    {SPI1_HW_FIXED_IO_C},
    {SPI1_HW_FIXED_IO_D}
...
static void spi1_iomc_config(hw_spi_dev spi, enum spi_mode mode)
{
...
#ifdef SPI_HW_FIXED_IO
    u8 i, j;
    for (i = 0; i < SPI1_HW_FIXED_IO_GROUP_NUM; i++) {
        for (j = 0; j < 5; j++) {
            if ((j > 1) && (port[j] == 0xff)) { //
                continue;
            }
            if (port[j] != spi1_fixed_io_tab[i][j]) {
                break;
            }
        }
        if (j == 5) {
    ...
    if (i == SPI1_HW_FIXED_IO_GROUP_NUM) {
        log_error("spi1 IO error!");
        return;
    }
    SPI1_HW_FIXED_IO_GROUP_SEL(i);
#else
...
#if SPI2_SUPPORT_UNIDIR_4BIT
        if (port[3] != (u8) - 1) {
            gpio_disable_function(IO_PORT_SPILT(port[3]), PORT_FUNC_SPI2_DA2);
        }
        if (port[4] != (u8) - 1) {
            gpio_disable_function(IO_PORT_SPILT(port[4]), PORT_FUNC_SPI2_DA3);
        }
#endif
#endif
    }
...
int spi_set_baud(hw_spi_dev spi, u32 baud)
{
    ...
    sysclk = clk_get("spi");
    log_debug("spi clock source freq %lu", (unsigned long)sysclk);
    if (sysclk < baud) {
...
int spi_open(hw_spi_dev spi, spi_hardware_info *spi_info)
{
    ...
    err = spi_set_baud(spi, spi_info->clk);
    if (err) {
    ...
    if (spi_info->ie_en == 1) {
        ...
        if (spi == HW_SPI1) {
            request_irq(IRQ_SPI1_IDX, spi_info->irq_priority, hw_spi1_isr, 0);//7:配置中断优先级，中断函数
#if SUPPORT_SPI2
        } else if (spi == HW_SPI2) {
            request_irq(IRQ_SPI2_IDX, spi_info->irq_priority, hw_spi2_isr, 0);//7:配置中断优先级，中断函数
#endif
        }
    }
 ...
 * @return null
 */
static volatile u8 *rec_data_byte[HW_SPI_MAX_NUM];
static volatile int spi_isr_data_len[HW_SPI_MAX_NUM];
 ...
 * @return null
 */
static volatile u8 *send_data_byte[HW_SPI_MAX_NUM];
static volatile int isr_addr_inc[HW_SPI_MAX_NUM];
AT(.spi.text.cache.L1)
...
volatile enum hw_spi_isr_status spi_isr_txrx_state[HW_SPI_MAX_NUM];
/*中断函数*/
AT(.spi.text.cache.L1)
