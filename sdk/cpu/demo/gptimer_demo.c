...
static void pwm_demo()
{
    ...
    //把一个timer_pwm 同时输出到多个IO上
    u8 signal = JL_OMAP->PC3_OUT;
    gpio_set_mode(PORTA, BIT(1), PORT_OUTPUT_LOW); //IO口设为输出
    JL_OMAP->PA1_OUT = signal;
    gpio_set_mode(PORTA, BIT(2), PORT_OUTPUT_LOW); //IO口设为输出
    JL_OMAP->PA2_OUT = signal;
    gpio_set_mode(PORTA, BIT(3), PORT_OUTPUT_LOW); //IO口设为输出
    JL_OMAP->PA3_OUT = signal;

    //释放
    gpio_set_mode(PORTA, BIT(1), PORT_HIGHZ); //IO口设为高阻
    JL_OMAP->PA1_OUT = 0;
    gpio_set_mode(PORTA, BIT(2), PORT_HIGHZ); //IO口设为高阻
    JL_OMAP->PA2_OUT = 0;
    gpio_set_mode(PORTA, BIT(3), PORT_HIGHZ); //IO口设为高阻
    JL_OMAP->PA3_OUT = 0;

    gptimer_deinit(id); //释放timer
}
#endif
...
//直接造作寄存器,实现特殊需求定时功能
#define         SYS_PLL_CLK_1DIV1_OE(x)     SFR(JL_PLL0->CON1, 11, 1,  x) //f1
#define         SYS_PLL_CLK_3DIV2_OE(x)     SFR(JL_PLL0->CON1, 12, 1,  x) //f2
#define         SYS_PLL_CLK_4DIV2_OE(x)     SFR(JL_PLL0->CON1, 13, 1,  x) //f3
#define         SYS_PLL_CLK_5DIV2_OE(x)     SFR(JL_PLL0->CON1, 14, 1,  x) //f4
#define         SYS_PLL_CLK_7DIV2_OE(x)     SFR(JL_PLL0->CON1, 15, 1,  x) //f5
static void custom_timer_demo()
{
    ...
    /* clk_out2(IO_PORTC_01, SYS_PLL_D2P5_CLK_OUT_2, 64); //理论输出时钟 480MHz/2.5/64 = 3MHz */
...
u32 timer_tid, pwm_tid, capture_tid;
void timer_callback_func(u32 tid, void *private_data)
{
    printf("gptimer timer 500mS callback\n");
}

void capture_callback_func(u32 tid, void *private_data)
{
    if (gptimer_get_work_mode(tid) == GPTIMER_MODE_CAPTURE_EDGE_FALL) {
        printf("gptimer captue PA0 faill edge !!!\n");
    }
}

void gptimer_test_demo_start(void)
{
    // timer模式
    const struct gptimer_config timer_config = {
        .timer.period_us = 500 * 1000,   //定时500ms
        .irq_cb = timer_callback_func, //回调函数
        .irq_priority = 1, 			   //中断优先级，默认给1
        .mode = GPTIMER_MODE_TIMER, //设置工作模式
        .private_data = NULL,
    };
    timer_tid = gptimer_init(TIMERx, &timer_config);
    gptimer_start(timer_tid);

    // pwm模式
    const struct gptimer_config pwm_config = {
        .pwm.port = PORTA, 			 //指定输出IO，PA组
        .pwm.pin = BIT(1), 				 //指定输出IO，BIT(1)脚  输出口PA1
        .pwm.freq = 1000,			 //输出频率1Khz
        .pwm.duty = 2000, //占空比20.00%
        .mode = GPTIMER_MODE_PWM, //设置工作模式
    };
    pwm_tid = gptimer_init(TIMERx, &pwm_config);
    gptimer_start(pwm_tid);

    // capture模式
    const struct gptimer_config capture_config = {
        .capture.port = PORTA, 					//指定捕捉IO，PA组
        .capture.pin = BIT(0), 						//指定捕捉IO，BIT(0)脚  捕捉口PA0
        .capture.filter = 0, 					//非红外接收默认写0，不支持非红外使用滤波功能
        .capture.max_period = 0,    //设置最大捕获时间
        .irq_cb = capture_callback_func,//回调函数
        .irq_priority = 1, 				//中断优先级，默认给1
        .mode = GPTIMER_MODE_CAPTURE_EDGE_FALL, //设置下降沿捕获
    };
    gpio_set_mode(IO_PORT_SPILT(IO_PORTA_00), PORT_INPUT_FLOATING);
    capture_tid = gptimer_init(TIMERx, &capture_config);
    gptimer_start(capture_tid);

    gptimer_dump();
}

void gptimer_test_demo_close(void)
{
    gptimer_deinit(timer_tid);
    gptimer_deinit(pwm_tid);
    gptimer_deinit(capture_tid);
}
