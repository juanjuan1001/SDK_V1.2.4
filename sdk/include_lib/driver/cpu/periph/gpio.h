...
// 内部信号中断
typedef void (*inside_signal_irq_callback_p)(enum inside_signal_sel sel, u32 null, enum gpio_irq_edge edge);
struct inside_signal_irq_config_st {
    enum gpio_irq_edge irq_edge;
    inside_signal_irq_callback_p callback;
    u8 irq_priority;//中断优先级
};
//配置中断(已使能)
//禁止同一个内部信号同一边沿多次注册
