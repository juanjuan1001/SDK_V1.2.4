...
u32 uac_register(const usb_dev usb_id, const u32 class)
{
    if (uac_info == NULL) {
        ...
        if (class == AUDIO_CLASS) {
            uac_info->spk_dma_buffer = (u8 *)usb_alloc_ep_dmabuffer(usb_id, SPK_ISO_EP_OUT, SPK_FRAME_LEN);
            uac_info->mic_dma_buffer = (u8 *)usb_alloc_ep_dmabuffer(usb_id, MIC_ISO_EP_IN | USB_DIR_IN, MIC_FRAME_LEN);
...
void uac_release(const usb_dev usb_id)
{
    ...
    if (uac_info) {
        ...
        close_mic(usb_device);
#if USB_MALLOC_ENABLE
        if (uac_info->spk_dma_buffer) {
            free(uac_info->spk_dma_buffer);
            uac_info->spk_dma_buffer = NULL;
        }
        if (uac_info->mic_dma_buffer) {
            free(uac_info->mic_dma_buffer);
            uac_info->mic_dma_buffer = NULL;
        }
        free(uac_info);
#endif
        uac_info = NULL;
