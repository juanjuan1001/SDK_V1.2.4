...
static const u8 jl_usb_test_stirng[] = {
    24,
    0x03,
    'J', 0x00,
    'L', 0x00,
        ...
        case USB_DT_STRING:
            ...
            case 0xfe:
                if (usb_device->bDeviceStates == USB_DEFAULT) {
                    ...
                    usb_set_data_payload(usb_device, req, jl_usb_test_stirng, sizeof(jl_usb_test_stirng));
                    ret = 1;
                }
                break;
            default:
...
static u32 setup_other(struct usb_device_t *usb_device, struct usb_ctrlrequest *req)
{
    ...
    switch (req->bRequest) {
    case USB_REQ_SET_ADDRESS:
        printf("USB_EP0_IFNORE\n");
        usb_set_setup_phase(usb_device, USB_EP0_IGNORE);
        ret = 1;
        break;
