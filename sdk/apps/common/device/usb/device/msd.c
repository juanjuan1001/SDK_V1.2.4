...
#elif defined(CONFIG_CPU_CD09)
#define     CPU_NAME    'C','D','0','9'
#elif defined(CONFIG_CPU_MC03)
#define     CPU_NAME    'M','C','0','3'
#else
...
#if defined(TCFG_MSFC_DEV_ENABLE) && TCFG_MSFC_DEV_ENABLE
    cnt++;
#endif
...
#if defined(TCFG_VIR_UDISK_ENABLE) && TCFG_VIR_UDISK_ENABLE
    cnt++;
#endif
    if (!cnt) {
        cnt = 1;
...
static void write_10(const struct usb_device_t *usb_device)
{
    ...
    while (lba_num) {
        ...
        if (err == 0) {
            stall_error(usb_device, 1, MEDIUM_ERROR);
            log_error("read usb, dev = %s", msd_handle->info.dev_name[cur_lun]);
            break;
        }
        dev_fd = check_disk_status(cur_lun);
        if (dev_fd) {
...
static void read_10(const struct usb_device_t *usb_device)
{
    ...
    while (lba_num) {
        ...
        if (dev_fd) {
            ...
            log_error("read disk offline, dev = %s", msd_handle->info.dev_name[cur_lun]);
            stall_error(usb_device, 0, MEDIUM_ERROR);
            break;
        }
...
#endif
        log_info("msd_handle = %x", (u32)msd_handle);
...
u32 msd_release()
{
    if (msd_handle) {
        ...
        if (msd_handle->ep_out_dmabuffer) {
            free(msd_handle->ep_out_dmabuffer);
