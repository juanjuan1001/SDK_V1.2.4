...
#if TCFG_USB_SLAVE_JL_TEST_ENABLE
    if ((class & JL_TEST_CLASS) == JL_TEST_CLASS) {
        log_info("add desc jl_usb_test");
        extern u32 jl_usb_test_desc_config(const usb_dev usb_id, u8 * ptr, u32 * cur_itf_num);
        extern u32 jl_usb_test_register(const usb_dev usb_id);
        usb_add_desc_config(usb_id, class_index++, jl_usb_test_desc_config);
        jl_usb_test_register(usb_id);
    }
#endif
