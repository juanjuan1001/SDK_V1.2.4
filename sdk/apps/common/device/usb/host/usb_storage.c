...
static int os_mutex_create(OS_MUTEX *m)
{
    return 0;
}
static int os_mutex_del(OS_MUTEX *m, u32 n)
{
    return 0;
}
static int os_mutex_pend(OS_MUTEX *m, u32 timeout)
{
    return 0;
}
static int os_mutex_post(OS_MUTEX *m)
{
    return 0;
}
static void os_time_dly(u32 time)
{
    mdelay(time);
}
...
static volatile u32 usb_h_mutex; // AT(.usb_stor.keep_ram);
...
static int usb_stor_ioctrl(struct device *device, u32 cmd, u32 arg)
{
    ...
    case IOCTL_SET_ASYNC_MODE:
        log_info(">> UDISK IOCTL_SET_ASYNC_MODE %d\n", arg);
#if (UDISK_READ_BIGBLOCK_ASYNC_ENABLE || UDISK_READ_512_ASYNC_ENABLE)
